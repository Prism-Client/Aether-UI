...
class UICircle(duration: Long = 1000L, animationDirection: UIEaseDirection = UIEaseDirection.INOUT) :
    ...
    override fun getValue(): Double {
        ...
        return when (animationDirection) {
            ...
            UIEaseDirection.INOUT -> if (x < 0.5) (1.0 - sqrt(1.0 - (2.0 * x).pow(2))) / 2.0 else (sqrt(
                1.0 - (-2.0 * x + 2.0).pow(
                    2
                )
            ) + 1.0) / 2.0
