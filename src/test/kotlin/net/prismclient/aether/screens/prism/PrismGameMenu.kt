...
class PrismGameMenu : UIScreen {
    ...
    override fun build() {
        build {
            ...
            style("icon-button") {
                size(189, 56)
                background(background_color_secondary, radius(15f))
            ...
            style("button") {
                size(161, 41)
                background(primary_color, radius(8f))
                ...
                list(UIListLayout.ListDirection.Vertical, UIListLayout.ListOrientation.Forward, "navbar-list") {
                    ...
                    selectable<UIImageButton> {
                        ...
                        onSelection {
                            ...
                            it.style.background = gradient {
                                radius(9f)
                                gradientWidth = rel(1f)
                                gradientHeight = rel(1f)
                                gradientStartColor = asRGBA(87, 164, 255)
                                gradientEndColor = asRGBA(84, 35, 167)
                            }
                        }
                        onDeselection {
                            it.hover("hover", "leave")
                            it.image.image = it.image.image.replace("-active", "")
                            it.style.background = UIBackground()
                            it.style.background {
                                backgroundColor = 0
                                radius(9f)
                            }
